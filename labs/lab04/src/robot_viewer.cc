...
RobotViewer::RobotViewer()
  ...
  gui->addButton("Color", std::bind(&RobotViewer::OnColorChangeBtnPressed, this));
...
void RobotViewer::DrawRobot(NVGcontext *ctx, int id, double xpos, double ypos,
  ...
  nvgFillColor(ctx, nvgRGBA(robot_land_->get_robot_colorx(id), robot_land_->get_robot_colory(id), robot_land_->get_robot_colorz(id), 255));
  nvgFill(ctx);
...
{
  int x=robot_land_->get_robot_colorx(0);
  int y=robot_land_->get_robot_colory(0);
  int z=robot_land_->get_robot_colorz(0);
  x=x+50;
  y=y+75;
  z=z+25;
  if(x>255)
  ...
  robot_land_->set_robot_color(0, x, y, z);
  //If you want to be cool, this bit swaps the colors
  /*int num_robots=robot_land_->get_num_robots();
  for(int i=0; i<num_robots; i++)
  {
    if((i+1)==num_robots)
    {
      robot_land_->set_robot_color(i, x, y, z);
    }
    else
    {
      robot_land_->set_robot_color(i, robot_land_->get_robot_colorx(i+1), robot_land_->get_robot_colory(i+1), robot_land_->get_robot_colorz(i+1));
    }
  }*/
}
