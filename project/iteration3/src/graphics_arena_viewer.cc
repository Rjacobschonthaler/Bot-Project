...
void GraphicsArenaViewer::DrawPlayer(
  ...
  nvgText(ctx, 0.0, 10.0, player->string_battery_level().c_str(), NULL);
...
void GraphicsArenaViewer::DrawSuperbot(
  ...
  // centered, at the position and heading for this robot
  nvgSave(ctx);
  nvgTranslate(ctx, superbot->get_pos().x, superbot->get_pos().y);
  nvgRotate(ctx, superbot->get_heading_angle());

  // robot's circle
  nvgBeginPath(ctx);
  nvgCircle(ctx, 0.0, 0.0, superbot->get_radius());
  ...
  nvgRotate(ctx, M_PI / 2.0);
  nvgFillColor(ctx, nvgRGBA(0, 0, 0, 255));
  nvgText(ctx, 0.0, 0.0, superbot->get_name().c_str(), NULL);
